...
计算hash值过程中，输入数据以`[BlockSize]byte`大小进行分组，凑够一组就能进行一次hash迭代，最终计算结果保存在一个`[Size]byte`大小的**数组**中返回，如果是`sha224`则需要对返回的数组进行截断，只返回前`28`个字节
...
根据方法签名，调用时需要传入待计算的字符串，返回值为一个`Size`或者`Size224`大小的`byte`数组
...
//output
//sha256: 86bda720f645a43e64f123e885577ab415d7146bf8fc7da3285ee5b99f52766f
//sha224: f37ba2701f3dee0267d068e2a86b164a03c6a7d7c498fdb32b04f09b
```
{% endcode %}
